...
| `decisionTable` | No wasted writes on a failing result | `ApplyResults` evaluates a rule's result value *before* calling `EnsurePath`/writing. A result that fails to evaluate never leaves an ensured-but-empty `{}` placeholder in the document, and under `allMatches` a later failing rule can no longer clobber an earlier rule's value by ensuring an empty container over it. | `TLio.Commands/DecisionTable.cs::ApplyResults`; [commands/DecisionTable.md#performance](commands/DecisionTable.md#performance) |
