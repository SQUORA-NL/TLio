...
3. **Given** a developer writes `"'=concat($.a, $.b)'"` with outer single quotes around a function, **When** they run the script, **Then** the system treats it as a literal string starting with `=concat...`, not a function call — and the notation reference explicitly warns against this pattern.
