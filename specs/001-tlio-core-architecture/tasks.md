...
> **Superseded (test project separation)**: XML tests moved to `TLio.Xml.Tests` by spec [003-xml-xpath-implementation](../003-xml-xpath-implementation/tasks.md).
> `XPathItemsFetcher` renamed `SlashPathItemsFetcher`; `NativeXPathItemsFetcher` added. Fixtures consolidated to single-file format.
...
- [x] Complete `SlashPathItemsFetcher.GetPath` — build slash-path from element lineage (renamed from `XPathItemsFetcher`)
- [x] Complete `SlashPathItemsFetcher.ResolveRelativePath`
- [x] Complete `SlashPathItemsFetcher.EnsurePath` and `SplitParentAndLeaf`
- [x] Add `XmlExecutionContext.CreateDefault()` / `CreateWithSlashPaths()` / `CreateWithNativeXPath()` factories
- [x] Implement `XmlScriptParser<TNode>` — parse XML-format scripts (`<script><set path="...">val</set></script>`)
- [x] Write XML fixture tests (Set, Add, Put, Remove, Copy, Move) — now in `TLio.Xml.Tests/Fixtures/XmlSet/` etc., single `fixture.xml` format
- [x] Write `XmlAdapterTests`
...
> **Superseded (test project separation)**: YAML tests moved to `TLio.Yaml.Tests` by spec [003-xml-xpath-implementation](../003-xml-xpath-implementation/tasks.md).
> Fixtures consolidated to single `fixture.yaml` (3-document format).

- [x] Implement `YamlParentTracker` — reference-equality dictionary for parent tracking
...
- [x] Complete `YamlNodeAdapter.Replace` — uses `YamlParentTracker`
