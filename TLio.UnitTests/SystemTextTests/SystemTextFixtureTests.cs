...
namespace TLio.UnitTests.SystemTextTests;

/// <summary>
/// Adapter compliance tests: runs the same fixture triplets as FixtureTests
/// but through the System.Text.Json adapter (SystemTextJsonExecutionContext).
///
/// Pass = System.Text.Json adapter is behaviorally equivalent to the Newtonsoft adapter
/// for the same inputs and scripts.
///
/// Each test method mirrors the corresponding method in FixtureTests but uses:
///   - FixtureTheoryLoader.LoadRaw (returns raw JSON strings instead of JToken)
///   - ParseOptions&lt;JsonNode&gt;.CreateDefault()
///   - SystemTextJsonExecutionContext.CreateDefault()
///   - JsonNode.DeepEquals for result comparison
/// </summary>
[TestFixture]
